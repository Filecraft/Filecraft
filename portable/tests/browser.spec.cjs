...
test('offline file workflow creates bounded PDF and invalidates edits',async({page,context},info)=>{
 ...
 await page.locator('#paper').selectOption('a4');await page.locator('#dpi').selectOption('150');await page.getByRole('button',{name:'Prepare PDF',exact:true}).click();await expect(page.locator('#status')).toContainText('Fits');
...
test('cancel discards late encode; grayscale pixels and padding are real',async({page},info)=>{
 ...
 await page.getByRole('button',{name:'Prepare PDF',exact:true}).click();await page.getByRole('button',{name:'Cancel processing',exact:true}).click();await expect(page.locator('#status')).toContainText('Cancelled');await expect(page.locator('#result')).toBeHidden();await expect(page.locator('#prepare')).toBeEnabled();
 await page.locator('#profile').selectOption('gray');await page.locator('#paper').selectOption('letter');await page.locator('#margin').fill('24');await page.locator('#dpi').selectOption('96');await page.getByRole('button',{name:'Prepare PDF',exact:true}).click();await expect(page.locator('#status')).toContainText('Fits');
...
test('rejects unsupported and oversized headers, cap, clear and impossible budget',async({page})=>{
 ...
 await page.locator('#limit').fill('0.01');await page.getByRole('button',{name:'Prepare PDF',exact:true}).click();await expect(page.locator('#status')).toContainText('Cannot fit');await expect(page.locator('#download')).toBeHidden();
