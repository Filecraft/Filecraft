...
test('offline file workflow creates bounded PDF and invalidates edits',async({page,context},info)=>{
 ...
 await page.locator('#paper').selectOption('a4');await page.locator('#dpi').fill('150');await page.getByRole('button',{name:'Prepare PDF',exact:true}).click();await expect(page.locator('#status')).toContainText('Fits');
...
test('cancel discards late encode; grayscale pixels and padding are real',async({page},info)=>{
 ...
 await page.locator('#profile').selectOption('gray');await page.locator('#paper').selectOption('letter');await page.locator('#margin').fill('24');await page.locator('#dpi').fill('96');await page.getByRole('button',{name:'Prepare PDF',exact:true}).click();await expect(page.locator('#status')).toContainText('Fits');
