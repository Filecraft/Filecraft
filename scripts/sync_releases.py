...
# Native installers are the primary consumer path; portable archives and browser
# packages remain catalogued for the releases that ship them.
ASSET_EXTENSIONS=('.zip','.dmg','.exe','.deb')
def asset_kind(name):
    if name.endswith('.dmg'):return 'macos installer'
    if name.endswith('.exe'):return 'windows installer'
    if name.endswith('.deb'):return 'linux package'
    return 'portable archive'
def asset_notes(name,platform):
    if name.endswith('.dmg'):return 'Open the DMG and drag Filecraft to Applications. Ad-hoc signed, not notarized.'
    if name.endswith('.exe'):return 'Run the installer, then open Filecraft from Start. Unsigned, so SmartScreen may warn.'
    if name.endswith('.deb'):return 'Install locally with: sudo apt install ./'+name
    if platform=='browser':return 'Local PDF workspace; extension stores not published.'
    return 'Self-contained desktop; optional OCR/media engines separate.'
assets=[]
for a in current['assets']:
    name=a['name']
    if not name.endswith(ASSET_EXTENSIONS):continue
    ...
    platform='windows' if 'windows' in name else 'linux' if ('linux' in name or name.endswith('.deb')) else 'macos' if ('darwin' in name or 'macos' in name or name.endswith('.dmg')) else 'browser'
    arch='Apple Silicon' if 'arm64' in name else 'Intel x64' if 'x86_64' in name else 'x64' if ('amd64' in name or 'x64' in name) else 'Chromium / Edge' if 'chromium' in name else 'Firefox temporary install' if 'firefox' in name else 'Web / offline ZIP'
    label=next((name[:-len(e)] for e in ASSET_EXTENSIONS if name.endswith(e)),name)
    assets.append({'name':name,'label':label,'kind':asset_kind(name),'url':a['browser_download_url'],'bytes':a['size'],'sha256':digest,'platform':platform,'architecture':arch,'version':args.current.removeprefix('v'),'notes':asset_notes(name,platform),'channel':'beta' if current['prerelease'] else 'stable','license':'Apache-2.0' if tuple(map(int,args.current.lstrip('v').split('-')[0].split('.'))) >= (0,9,0) else 'historical terms'})
assets.sort(key=lambda a:(a['platform'],a['architecture']))
args.site.joinpath('downloads.json').write_text(json.dumps(assets,indent=2)+'\n')
print('Actual releases',len(releases),'current assets',len(assets))
