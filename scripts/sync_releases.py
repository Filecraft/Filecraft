...
releases=json.loads(subprocess.check_output(['gh','api','repos/Filecraft/Filecraft/releases?per_page=100'],text=True))
...
for a in current['assets']:
    ...
    assets.append({'name':name,'label':name.removesuffix('.zip'),'url':a['browser_download_url'],'bytes':a['size'],'sha256':digest,'platform':platform,'architecture':arch,'version':args.current.removeprefix('v'),'notes':'Self-contained desktop; optional OCR/media engines separate.' if platform!='browser' else 'Local PDF workspace; extension stores not published.', 'channel':'beta' if current['prerelease'] else 'stable','license':'Apache-2.0' if args.current not in ['v0.8.0-beta.1'] else 'historical terms'})
