...
Requires macOS Accessibility permission (CGPreflightPostEventAccess) for the
process that runs this script, and a process started after that grant. It never
grants itself permission, never disables Gatekeeper and never asks for a
password. Run it on an unlocked desktop.
...
def main() -> int:
    ...
    driver = QuartzDriver()
    permission = bool(driver.q.CGPreflightPostEventAccess()) if hasattr(driver.q, 'CGPreflightPostEventAccess') else False
    if not hasattr(driver.q, 'CGPreflightPostEventAccess'):
        import ctypes
        permission = bool(ctypes.cdll.LoadLibrary(
            '/System/Library/Frameworks/CoreGraphics.framework/CoreGraphics').CGPreflightPostEventAccess())
    record['post_event_access'] = permission
    ...
    try:
        ...
        if not permission:
            record['status'] = ('BLOCKED: CGPreflightPostEventAccess is false, so OS input cannot '
                                'be delivered. Grant Accessibility to the host application and '
                                'restart it, then rerun.')
            proc.terminate()
            proc.wait(timeout=15)
            (args.evidence / 'result.json').write_text(json.dumps(record, indent=2) + '\n')
            print(record['status'])
